...
subject: "Bayesian Modeling Applications"
  """
    ...
    def calculate_precision_recall(self, gt_records: List[Dict], pred_records: List[Dict], gt_attr: str, pred_attr: str) -> Tuple[float, float, float]:
        """
        ...
        precision, recall, f1, _ = precision_recall_fscore_support(aligned_gt_values, aligned_pred_values, average='weighted', zero_division=0)
    ...
    def get_valid_records(self, lod: List[Dict], pkey_attr: str, attr: str, na: str = "N/A") -> Dict[str, Dict]:
        """
    ...
    def stat_availability(self, stat: Dict[str, float], prefix: str, lod: List[any], valid_lod: List[any]) -> float:
        """
    ...
    def test_collect_and_parse_yaml_files(self):
        ...
        for event_attr, vol_attr in self.event_attribute_map.items():
            ...
            if vol_attr:
                stat["attr"]=attr
                avail=self.stat_availability(stat, "event",unique_events, event_records.values())
                avails.append(avail)
                ...
                if vol_attr and event_attr:
                    precision, recall, f1 = self.calculate_precision_recall(volume_records, event_records, vol_attr, event_attr)
