...
class TestSync(Basetest):
    ...
    def test_Sync(self):
        ...
        assert sync.get_keys("→") == {"d", "e"}, "Mismatch in keys present only in local data"
    
        # Assert keys that are in both local (left) and wikidata (right)
        assert sync.get_keys("↔") == {"a", "b"}, "Mismatch in keys present in both data sets"
        ...
        assert local_record_by_key is not None and local_record_by_key["lvalue"] == local_test_value, "get_record_by_key failed for local data"
        ...
        assert wikidata_record_by_key is not None and wikidata_record_by_key["wvalue"] == wikidata_test_value, "get_record_by_key failed for wikidata"
        ...
        assert local_pkey_record is not None and local_pkey_record["id_l"] == local_pkey, "get_record_by_pkey failed for local data"
        assert wikidata_pkey_record is not None and wikidata_pkey_record["id_r"] == wikidata_pkey, "get_record_by_pkey failed for wikidata"
